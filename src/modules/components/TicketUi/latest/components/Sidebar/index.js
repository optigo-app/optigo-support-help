import { Box, List, ListItem, ListItemButton, ListItemText, Typography, Divider, IconButton, Tooltip, Menu, MenuItem, useMediaQuery } from "@mui/material";
...
const Sidebar = ({ handleCreateTicket, activeItem, setActiveItem, tickets, AgesBasedFilter, setAgesBasedFilter }) => {
	...
	useLayoutEffect(() => {
		if (isSmallScreen) {
			setCollapsed(true);   // Auto collapse on small devices
		} else {
			setCollapsed(false);  // Expand on large devices
		}
	}, [isSmallScreen]);
