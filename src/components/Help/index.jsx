...
import ViDashboardSections from "./New";
...
const Main = ({ faqPathInfo }) => {
  ...
  const showOnlyFaq =
    typeof showMore === "string"
      ? ["1", "true"].includes(showMore.toLowerCase())
      : Boolean(showMore);
    ...
    <>
      <Box sx={{ backgroundColor: "#fff", minHeight: "100vh" }}>
        ...
        <Container maxWidth="lg" sx={{ py: 6 }}>
          ...
          <QuickGuide
            queryWords={
              hasActiveSearch && [...new Set(activeSearchQuery.split(" "))]
            }
            faqData={faqDataToShow}
            showTitle={!showOnlyFaq}
            shouldShowOtherComponents={shouldShowOtherComponents}
            SearchResults={SearchResults}
            attachImageClickHandlers={attachImageClickHandlers}
            handleImageClick={handleImageClick}
            expanded={expanded}
            handleChange={handleChange}
            searchResults={hasActiveSearch ? SearchResults : null}
