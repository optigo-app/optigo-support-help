...
import {
    Container, Box, Typography, Button, ThemeProvider, createTheme, CssBaseline,
    List, ListItem, ListItemText, ListItemButton, Divider, Card, CardContent, Grid, Popover,
...
const theme = createTheme({
    ...
    components: {
        MuiCard: {
            styleOverrides: {
                root: { borderRadius: 16, borderColor: '#F3F4F6', boxShadow: 'none', border: '1px solid #E5E7EB' },
            },
        },
    },
});
...
const bentoArticles = [
    {
        ...
        color: '#EC4899',
        span: 4,
        description: 'Master every shortcut to navigate faster and work smarter without touching your mouse.',
        readTime: '2 min read',
        icon: '⌨️',
    },
    {
        ...
        color: '#8B5CF6',
        span: 8,
        description: 'Connect Optigo with your existing tools — Slack, Jira, GitHub, and 40+ other platforms via our REST API and webhooks.',
        readTime: '8 min read',
        icon: '🔗',
    },
    {
        ...
        color: '#6D28D9',
        span: 8,
        description: 'Full reference for the Optigo REST API: authentication, endpoints, rate limits, and step-by-step code examples in JS, Python & cURL.',
    ...
    },
    {
        ...
        color: '#3B82F6',
        span: 4,
        description: 'The most common questions answered by Optigo experts and power users.',
        readTime: '4 min read',
        icon: '💬',
    },
];
...
export default function HelpArticle() {
    ...
    const handleFaqClick = (e) => {
        if (e.target.tagName === 'IMG') {
            ...
            let index = allImages.findIndex((img) => src.includes(img.src) || img.src.includes(src));
            if (index < 0) {
    ...
    const pageTitle = React.useMemo(() => {
        ...
        return (lang === 'hi' && currentItem.titleHindi) ? currentItem.titleHindi : currentItem.title;
    }, [currentItem, title, lang]);

    const pageDesc = React.useMemo(() => {
        if (!currentItem) return 'Learn the essential skills to master this topic dynamically.';
        return (lang === 'hi' && currentItem.descHindi) ? currentItem.descHindi : (currentItem.desc || 'Learn the essential skills to master this topic dynamically.');
    }, [currentItem, lang]);
    ...
    const availableLangs = React.useMemo(() => {
        ...
        if (currentItem?.youtubeIdEnglish)
            langs.push({ code: 'en', label: 'English', flag: '🇬🇧', videoId: currentItem.youtubeIdEnglish });
        if (currentItem?.youtubeIdHindi)
            langs.push({ code: 'hi', label: 'हिंदी', flag: '🇮🇳', videoId: currentItem.youtubeIdHindi });
        // Fallback: use generic youtubeId as English if no language-specific IDs
        if (langs.length === 0 && currentItem?.youtubeId)
            langs.push({ code: 'en', label: 'English', flag: '🇬🇧', videoId: currentItem.youtubeId });
        return langs;
    }, [currentItem]);
    ...
    const recommended = React.useMemo(() => {
        ...
        const same = allItems.filter(i => i.slug !== slug && (i.section === currentGroup || i.category === currentGroup) && hasVideo(i));
        const other = allItems.filter(i => i.slug !== slug && i.section !== currentGroup && i.category !== currentGroup && hasVideo(i));
    ...
    const relatedFaqs = React.useMemo(() => {
        ...
        return FaqList.filter(faq => {
            const text = [faq.question, faq.category, ...(faq.tags || [])].join(' ').toLowerCase();
    ...
    const generateChapters = React.useCallback((totalSeconds) => {
        const LABELS = ['Introduction', 'Getting Started', 'Core Concepts', 'Step by Step', 'Tips & Shortcuts', 'Summary & Next Steps'];
        const count = totalSeconds < 180 ? 3 : totalSeconds < 360 ? 4 : totalSeconds < 600 ? 5 : 6;
        ...
        progressTimer.current = setInterval(() => {
            ...
            try {
                ...
                for (let i = ch_list.length - 1; i >= 0; i--) {
                    if (cur >= ch_list[i].seconds) { ch = i; break; }
                }
    ...
    React.useEffect(() => {
        if (showLangPrompt) return; // Wait for language selection

        let cancelled = false;

        loadYTScript().then(() => {
            if (cancelled || !iframeRef.current || ytPlayer.current) return;

            ytPlayer.current = new window.YT.Player(iframeRef.current, {
                videoId: availableLangs.find(l => l.code === lang)?.videoId || availableLangs[0]?.videoId || '',
                width: '100%',
                height: '100%',
                playerVars: {
                    controls: 0,
                    ...
                    fs: 0,
                    playsinline: 1,
                    origin: window.location.origin,  // required for embed trust
                    enablejsapi: 1,
    ...
    }, [showLangPrompt]); // Re-run when prompt is dismissed
    ...
    const togglePlay = () => {
        ...
        try {
            ...
            if (!YT_STATE) return;
            // PLAYING(1) or BUFFERING(3) → pause; anything else → play
    ...
    const handleLangSwitch = (langCode) => {
        if (!ytPlayer.current || langCode === lang) {
            setLangAnchor(null);
            return;
        }
    ...
    return (
        <ThemeProvider theme={theme}>
            ...
            <Box sx={{ bgcolor: '#ffffff', minHeight: '100vh', py: { xs: 2, md: 4 } }}>
                <Container sx={{ maxWidth: { xs: '100%', sm: '100%', md: '1200px', lg: '1600px', xl: '1600px' }, margin: '0 auto' }}>

                    <Box sx={{ mb: 2 }}>
                        <Button
                            startIcon={<ArrowBackIcon fontSize="small" />}
                            onClick={() => navigate(-1)}
                            sx={{ color: '#6B7280', textTransform: 'none', borderRadius: 8, px: 2, '&:hover': { bgcolor: '#F3F4F6' } }}
                        >
                            Back
                        </Button>
                    </Box>

                    <Box sx={{ display: 'flex', gap: { xs: 3, md: 4 }, alignItems: 'flex-start', flexDirection: { xs: 'column', md: 'row' } }}>

                        {/* ── LEFT: Video + Recs + Bento ── */}
                        <Box sx={{ flexGrow: 1, minWidth: 0, pb: 8, width: { xs: '100%', md: 'calc(100% - 340px)' } }}>
                            ...
                            <Box
                                ...
                                sx={{
                                    width: '100%', aspectRatio: '16/9', bgcolor: '#111827',
                                    borderRadius: 4, overflow: 'hidden', mb: 5,
                                    position: 'relative', cursor: showLangPrompt ? 'default' : 'pointer',
                            ...
                            >
                                ...
                                {showLangPrompt && (
                                    <Box sx={{
                                        ...
                                        <Box sx={{
                                            ...
                                            <Typography variant="h6" sx={{ color: '#111827', mb: 3, fontWeight: 800 }}>
                                ...
                                {!showLangPrompt && (
                                    ...
                                    >
                                        <Box sx={{
                                            ...
                                            {isPlaying
                                                ? <PauseRoundedIcon sx={{ fontSize: 55, color: '#fff' }} />
                                                : <PlayArrowRoundedIcon sx={{ fontSize: 55, color: '#fff' }} />
                                            }
                                        </Box>
                                    </Box>
                                )}
                                {!showLangPrompt && (
                                    <Box sx={{
                                        ...
                                        pt: 5, pb: 1.5, px: 2,
                                        opacity: (!isPlaying || isHovered) ? 1 : 0,
                                        transform: (!isPlaying || isHovered) ? 'translateY(0)' : 'translateY(6px)',
                                        ...
                                        <Slider
                                            ...
                                            sx={{
                                                ...
                                                '& .MuiSlider-thumb': {
                                                    ...
                                                    '&:hover': { boxShadow: '0 0 0 8px rgba(139,92,246,0.2)' },
                                                },
                                        ...
                                        <Box sx={{ display: 'flex', alignItems: 'center', mt: 0.5, gap: 0.5 }}>
                                            <IconButton onClick={togglePlay} size="small"
                                                sx={{ color: '#fff', bgcolor: 'rgba(255,255,255,0.12)', borderRadius: '50%', '&:hover': { bgcolor: 'rgba(255,255,255,0.25)' } }}>
                                                {isPlaying ? <PauseRoundedIcon fontSize="small" /> : <PlayArrowRoundedIcon fontSize="small" />}
                                            </IconButton>

                                            <IconButton onClick={handlePrevChapter} size="small"
                                                sx={{ color: '#fff', borderRadius: '50%', '&:hover': { bgcolor: 'rgba(255,255,255,0.18)' } }}>
                                                <SkipPreviousIcon fontSize="small" />
                                            </IconButton>

                                            <IconButton onClick={handleNextChapter} size="small"
                                                sx={{ color: '#fff', borderRadius: '50%', '&:hover': { bgcolor: 'rgba(255,255,255,0.18)' } }}>
                                                <SkipNextIcon fontSize="small" />
                                            </IconButton>

                                            <Typography variant="caption"
                                                sx={{ color: '#fff', ml: 1, opacity: 0.85, fontWeight: 500, letterSpacing: 0.3 }}>
                                            ...
                                            <Box
                                                ...
                                                sx={{
                                                    ...
                                                    px: 1.5, py: 0.5,
                                                    borderRadius: 999,
                                                    bgcolor: langAnchor ? 'rgba(255,255,255,0.28)' : 'rgba(255,255,255,0.12)',
                                            ...
                                            >
                                                ...
                                                <Typography variant="caption" sx={{
                                                    color: '#fff', fontWeight: 700, fontSize: '0.72rem', letterSpacing: 0.4,
                                                }}>
                                                    {availableLangs.find(l => l.code === lang)?.label}
                                                </Typography>
                                                <Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.55)', fontSize: '0.6rem', ml: 0.2 }}>▾</Typography>
                                            </Box>
                                                        ...
                                                        return (
                                                            <Box
                                                                ...
                                                                sx={{
                                                                    ...
                                                                    '&:hover': { bgcolor: isActive ? '#FFE8D6' : '#F9FAFB' },
                                                                }}
                                                            >
                                                                <Typography sx={{ fontSize: '1.1rem', lineHeight: 1, minWidth: 24 }}>{l.flag}</Typography>
                                            ...
                                            >
                                                <AccordionSummary
                                                    expandIcon={
                                                        ...
                                                        >
                                                            {faqExpanded === index ? <RemoveIcon sx={{ fontSize: 16 }} /> : <AddIcon sx={{ fontSize: 16 }} />}
                                                        </Box>
                                                    }
                                                    sx={{
                                                        minHeight: "auto",
                                                ...
                                                >
                                                    <Typography sx={{ fontWeight: 500, color: "#1a1a1a", fontSize: { xs: 16, md: 17 } }}>
                                                        {faq.question}
                                                    </Typography>
                                                </AccordionSummary>
                                                {faq.answer && (
                                                    <AccordionDetails sx={{ px: 0, pt: 0, pb: 2 }}>
                                                        <Box 
                                                            sx={{ 
                                                                color: "#878787", lineHeight: 1.6, fontSize: { xs: 15, md: 16 },
                                                                '& img': { cursor: 'zoom-in', maxWidth: '100%', height: 'auto', display: 'block', mt: 2, mb: 2, borderRadius: 2 } 
                                                            }} 
                            ...
                            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3 }}>
                                <Typography variant="h5">{lang === 'hi' ? "अनुशंसित वीडियो" : "Recommended Videos"}</Typography>
                                <Button size="small" sx={{ color: '#8B5CF6', textTransform: 'none', fontWeight: 600, '&:hover': { bgcolor: '#F5F3FF' } }}>{lang === 'hi' ? "सभी देखें →" : "View all →"}</Button>
                            </Box>
                            <Grid container spacing={2.5} sx={{ mb: 6 }}>
                                {recommended?.map((video, idx) => {
                                    const vId = video.youtubeIdEnglish || video.youtubeIdHindi || video.youtubeId;
                                    return (
                                        <Grid item xs={12} sm={6} key={idx}>
                                            <Card 
                                                ...
                                                sx={{
                                                    ...
                                                    '&:hover': { borderColor: '#8B5CF6', transform: 'translateY(-3px)', boxShadow: '0 8px 24px rgba(139,92,246,0.12)' }
                                                }}>
                                                {/* Thumbnail */}
                                                <Box sx={{ position: 'relative', overflow: 'hidden' }}>
                                                    <Box
                                                        ...
                                                        alt={(lang === 'hi' && video.titleHindi) ? video.titleHindi : video.title}
                                                    ...
                                                    <Box sx={{
                                                        ...
                                                        fontSize: '0.7rem', fontWeight: 700, fontFamily: 'inherit', letterSpacing: 0.3,
                                                    }}>
                                                        {video.duration || '5:00'}
                                                    </Box>
                                                    {/* Category badge */}
                                                    <Box sx={{
                                                        ...
                                                        fontSize: '0.65rem', fontWeight: 700, fontFamily: 'inherit', letterSpacing: 0.5, textTransform: 'uppercase',
                                                    }}>
                                                        {(lang === 'hi' && video.sectionHindi) ? video.sectionHindi : video.section}
                                                    </Box>
                                                </Box>
                                                <CardContent sx={{ p: 2, flexGrow: 1 }}>
                                                    <Typography variant="body2" sx={{ fontWeight: 700, mb: 0.75, color: '#111827', lineHeight: 1.4, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{(lang === 'hi' && video.titleHindi) ? video.titleHindi : video.title}</Typography>
                                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, color: '#9CA3AF' }}>
                                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.4 }}>
                                                            <AccessTimeIcon sx={{ fontSize: 13 }} />
                                                            <Typography variant="caption" sx={{ fontSize: '0.72rem' }}>{video.duration || '5:00'}</Typography>
                                                        </Box>
                                                        <Typography variant="caption" sx={{ fontSize: '0.72rem' }}>·</Typography>
                                                        <Typography variant="caption" sx={{ fontSize: '0.72rem' }}>{video.views || '1K'}</Typography>
                                                        <Typography variant="caption" sx={{ fontSize: '0.72rem' }}>·</Typography>
                                                        <Typography variant="caption" sx={{ fontSize: '0.72rem' }}>{video.author || 'Optigo Support'}</Typography>
                                                    </Box>
                                                </CardContent>
                                            </Card>
                                        </Grid>
                        ...
                        <Box sx={{ width: { xs: '100%', md: 320 }, minWidth: { md: 320 }, position: { md: 'sticky' }, top: 80, display: 'flex', flexDirection: 'column', gap: 2 }}>

                            {/* Chapters */}
                            <Box sx={{ borderRadius: 4, border: '1px solid #E5E7EB', bgcolor: '#fff', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.05)', p: 2.5 }}>
                                <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                                    <BookmarkBorderIcon fontSize="small" sx={{ color: '#4B5563', mr: 1 }} />
                                    <Typography variant="subtitle2" sx={{ fontWeight: 700, color: '#111827', textTransform: 'uppercase', letterSpacing: 0.5 }}>
                                            ...
                                            <ListItem key={idx} disablePadding sx={{ mb: 0.25 }}>
                                                ...
                                                >
                                                    <Box sx={{ width: 45, flexShrink: 0 }}>
                                                        <Typography variant="caption" sx={{
                                                            color: sel ? '#fff' : '#8B5CF6', fontWeight: 700,
                                                    ...
                                                    <ListItemText
                                                        primary={ts.label}
                                                        primaryTypographyProps={{ variant: 'caption', sx: { pl: 0.5, fontWeight: sel ? 700 : 500, color: sel ? '#6D28D9' : '#4B5563', lineHeight: 1.2 } }}
                                                    />
                                                </ListItemButton>
                                            </ListItem>
                                        );
                            ...
                            <Box sx={{ borderRadius: 4, border: '1px solid #E5E7EB', bgcolor: '#fff', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.05)', p: 2.5 }}>
                                ...
                                <List disablePadding>
                                    {['Getting Started Guide', 'Support & FAQ', 'Community Forums', 'Submit a Ticket'].map((link, idx) => {
                                        ...
                                        return (
                                            <ListItem key={idx} disablePadding sx={{ mb: 0.25 }}>
                                                <ListItemButton
                                                    ...
                                                    sx={{
                                                        borderRadius: 2, py: 0.5, px: 1,
                                                        '&.Mui-selected': { bgcolor: '#FFF1F2', color: '#E11D48' },
                                                ...
                                                >
                                                    <ListItemText
                                                        primary={link}
                                                        primaryTypographyProps={{ variant: 'caption', sx: { fontWeight: sel ? 700 : 500, color: 'inherit', lineHeight: 1.2 } }}
                                                    />
                                                    <OpenInNewIcon sx={{ fontSize: 14, color: sel ? '#E11D48' : '#9CA3AF' }} />
                                                </ListItemButton>
                                            </ListItem>
                                        );
                                    })}
